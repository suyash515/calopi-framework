...
class ScriptJavascriptFileWriter extends JavascriptFileWriter
{
	...
	$fieldList = $tableEntity->getNonKeyParameterList();
	...
	$this->appendFunctionDeclaration($functionName, "id");
	$this->openCurly();
...
//	$parameterString = $tableEntity->getJavascriptCommaPrimaryParameterList();
	$parameterString = "id";
	$allFieldList = $tableEntity->getNonPrimaryParameterList();

	$this->appendFunctionDeclaration($functionName, $parameterString);
	$this->openCurly();
	$this->addContent("var con = $editContainerText + $parameterString;");
	$this->addEmptyLine();
	...
	for($i = 0; $i < count($fieldList); $i++)
	{
	    ...
	    $this->addContent("var $fieldVariableName = \$($fieldId + ".$parameterString.").val();");
	}
