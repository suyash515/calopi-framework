...
class BaseEntityPhpFileWriter extends PHPFileWriter
{
    ...
    protected function appendGetEntityFunctions()
    {
	$fieldEntityList = $this->tableEntity->getForeignKeyParameterList();

	for($i = 0; $i < count($fieldEntityList); $i++)
	{
	    $fieldName = $fieldEntityList[$i]->getField();

	    $getterFunctionName = "get".TextUtility::formatVariableNameWithFirstLetterCapitalised($fieldName)."Entity";
	    $fileName = BaseEntityPhpFileWriter::getBaseLogicNameFromForeignKey($fieldName);

	    $this->addContent("public function $getterFunctionName()");
	    $this->openCurly();
	    $this->addContent("return $fileName::convertToObject(\$this->getValues());");
	    $this->closeCurly();
	    $this->addEmptyLine();
	}
    }

    public static function getBaseLogicNameFromForeignKey($fieldName)
    {
	$formattedFieldName = str_replace("fk_", "", $fieldName);
	$formattedFieldName = str_replace("_id", "", $formattedFieldName);

	return PHPFileWriterManager::getBaseLogicName($formattedFieldName);
    }
