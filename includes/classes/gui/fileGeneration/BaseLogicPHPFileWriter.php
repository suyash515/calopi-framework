...
class BaseLogicPHPFileWriter extends PHPFileWriter
{
	...
	$this->addEmptyLine();
	$this->appendGetSpecificDetailsFunction();
	$this->addEmptyLine();
	$this->appendGetSpecificFieldValuesFunctions();
	$this->addEmptyLine();
	$this->appendUpdateSpecificDetailsFunction();
	$this->addEmptyLine();
	$this->appendUpdateSpecificFieldValuesFunctions();
	$this->addEmptyLine();
    ...
    public function getFieldEnumVariableName($fieldName, $enum)
    {
    ...
    protected function appendGetSpecificDetailsFunction()
    {
	$parameterList = $this->getSpecificDetailsFunctionParameterList();
	$functionName = BaseLogicPHPFileWriter::getGetSpecificDetailsFunctionName();
	$fileName = $this->fileName;

	$this->appendFunctionDeclaration($functionName, "", "", true, $parameterList, parent::$MODIFIER_PROTECTED);
	$this->openCurly();

	$this->addContent("\$queryBuilder = new QueryBuilder();");
	$this->addContent("\$queryBuilder->addTable($fileName::\$TABLE_NAME);");
	$this->addContent("\$queryBuilder->addField(\$field);");
	...
	for($i = 0; $i < count($fieldEntityList); $i++)
	{
	    ...
	    $fieldNameVariable = $this->getFieldVariableName($fieldName);
	    $valueVariable = PHPFileWriter::createVariable(TextUtility::formatVariableName($fieldName));
    ...
    protected function appendGetSpecificFieldValuesFunctions()
    {
	$fieldEntityList = $this->tableEntity->getFieldEntityList();
	$parameterList = $this->tableEntity->getCommaPrimaryParameterList();
	$fileName = $this->fileName;

	for($i = 0; $i < count($fieldEntityList); $i++)
	{
	    $functionName = $fieldEntityList[$i]->getGetterFunctionName();
	    $fieldNameVariable = $this->getFieldVariableName($fieldEntityList[$i]->getField());

	    $this->appendFunctionDeclaration($functionName, "", "", true, $parameterList);
	    $this->openCurly();
	    $this->addContent("return BaseBannerLogicUtility::getSpecificDetails($parameterList, $fileName::$fieldNameVariable);");
	    $this->closeCurly();
    ...
    protected function appendUpdateSpecificDetailsFunction()
    {
	$parameterList = $this->getParameterListForUpdate();
	$functionName = BaseLogicPHPFileWriter::getUpdateSpecificDetailsFunctionName();
	$fileName = $this->fileName;
	...
	$this->addContent("\$queryBuilder = new QueryBuilder();");
	$this->addContent("\$queryBuilder->addTable($fileName::\$TABLE_NAME);");
	$this->addContent("\$queryBuilder->addUpdateField(\$field, \$value, QueryBuilder::\$DOUBLE_QUOTE);");
	...
	$this->addEmptyLine();
	$this->addContent("\$result = \$queryBuilder->executeUpdateQuery();");
	$this->closeCurly();
    }

    protected function appendUpdateSpecificFieldValuesFunctions()
    {
	$fieldEntityList = $this->tableEntity->getNonPrimaryParameterList();
	$parameterList = $this->getParameterListForSpecificUpdate();
	$fileName = $this->fileName;
	$updateFunctionName = Configuration::$UPDATE_SPECIFIC_FIELD_FUNCTION_NAME;

	for($i = 0; $i < count($fieldEntityList); $i++)
	{
	    $functionName = $fieldEntityList[$i]->getUpdateFunctionName();
	    $fieldNameVariable = $this->getFieldVariableName($fieldEntityList[$i]->getField());

	    $this->appendFunctionDeclaration($functionName, "", "", true, $parameterList);
	    $this->openCurly();
	    $this->addContent("$fileName::$updateFunctionName($parameterList, $fileName::$fieldNameVariable, \$value);");
	    $this->closeCurly();
    ...
    protected function getParameterListForUpdate()
    {
	$parameterList = $this->tableEntity->getCommaPrimaryParameterList();

	return $parameterList.", \$field, \$value";
    }

    protected function getParameterListForSpecificUpdate()
    {
	$parameterList = $this->tableEntity->getCommaPrimaryParameterList();
	...
	$entityClassName = PHPFileWriterManager::getEntityName($this->tableEntity->getTableName());
    ...
    public function getSpecificDetailsFunctionParameterList()
    {
	$primaryParameter = $this->tableEntity->getCommaPrimaryParameterList();

	return $primaryParameter.", \$field";
    }
    ...
    public static function getGetSpecificDetailsFunctionName()
    {
	return Configuration::$GET_SPECIFIC_DETAILS_FUNCTION_NAME;
    }

    public static function getUpdateSpecificDetailsFunctionName()
    {
	return Configuration::$UPDATE_SPECIFIC_FIELD_FUNCTION_NAME;
    }
